...
// Is Use ESLint
const shouldESLint = process.env.ESLINT !== 'false';

// If build Dll or build Lib, is only build Javascript
const isOnlyBuildJS = shouldBuildDll || shouldBuildLib;

// If isOnlyBuildJS, is not use dll
const shouldUseDll = isOnlyBuildJS ? false : process.env.USE_DLL !== 'false';
...
// fix dll datas
const dll = {};
const dllIncludeKeys = [];
const dllIncludePaths = [];
const dllIncludeRandomPaths = [];
for (const dllKey in pkg) {
  if (/^dll_/.test(dllKey)) {
    for (const k in pkg[dllKey]) {
      if (pkg[dllKey][k]) {
        if (!dll[dllKey]) {
          dll[dllKey] = [];
          dllIncludeKeys.push(dllKey);
          dllIncludePaths.push(`dll/${dllKey}.js`);
          dllIncludeRandomPaths.push(`dll/${dllKey}${paths.randomId}.js`);
        }
        dll[dllKey].push(k);
      }
    }
...
const dllDirPath = path.resolve(__dirname, '../public/dll');

// make build dll plugin
const buildDllPlugin =
  shouldBuildDll &&
...
// make use dll plugin
const useDllReferencePlugin = shouldUseDll
  ? dllIncludeKeys.map(
      name =>
        new webpack.DllReferencePlugin({
          context: '.',
          manifest: require(path.resolve(dllDirPath, `${name}-manifest.json`)),
        }),
    )
  : [];
  ...
  return {
    ...
    ...(shouldBuildLib && {
      externals: [
        ...
        //   if (/^src\/ExampleComponents\//.test(request)) {
        //     const requestObjPath = request.split(/^src\/ExampleComponents\//)[1];
    ...
    output: {
      ...
      filename: isOnlyBuildJS
        ? '[name].js'
        : isEnvProduction
        ? 'static/js/[name].[chunkhash:8].js'
        : isEnvDevelopment && 'static/js/bundle.js',
      // There are also additional JS chunk files if you use code splitting.
      chunkFilename: isOnlyBuildJS
        ? undefined
        : isEnvProduction
      ...
      devtoolModuleFilenameTemplate: isOnlyBuildJS
        ? undefined
        : isEnvProduction
    ...
    plugins: [
      ...
      !isOnlyBuildJS &&
        new HtmlWebpackIncludeAssetsPlugin({
          assets: isEnvDevelopment ? dllIncludePaths : dllIncludeRandomPaths,
          append: false,
        }),
